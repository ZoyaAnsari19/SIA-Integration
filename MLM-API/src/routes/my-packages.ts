...
export async function myPackagesRoutes(app: FastifyInstance) {
  /**
   * @openapi
   * /api/v1/my-packages:
   *   get:
   *     tags:
   *       - My Packages
   *     summary: Get user package purchase history
   *     description: |
   *       Retrieve all package purchases for the authenticated user.
   *       Includes direct packages, course purchases, and manual deposits.
  ...
  app.get('/', {
    ...
    schema: {
      description: 'Get user package purchase history (all types)',
      tags: ['My Packages'],
            ...
            items: {
              ...
              items: {
                ...
                properties: {
                  ...
                  amount: { type: 'number' },
                  income: { type: 'number' }, // Income earned (for 2x progress)
                  purchased_at: { type: 'string', format: 'date-time' },
                  // active_until removed - expiry is ONLY based on 2x income
                  status: { type: 'string' },
                  is_active: { type: 'boolean' },
                  is_renewal: { type: 'boolean' },
                  previous_package_id: { type: ['number', 'null'], nullable: true },
                  previous_purchase_id: { type: ['string', 'null'], nullable: true },
                  global_ids_info: {
                    type: ['object', 'null'],
                    nullable: true,
                    properties: {
    ...
    try {
      ...
      const items = await Promise.all(
        purchases.map(async (p) => {
          ...
          // Convert previous_purchase_id to string (handle BigInt)
          let previousPurchaseId: string | null = null;
          if (p.previous_purchase_id !== null && p.previous_purchase_id !== undefined) {
            if (typeof p.previous_purchase_id === 'bigint') {
              previousPurchaseId = p.previous_purchase_id.toString();
            } else {
              previousPurchaseId = String(p.previous_purchase_id);
            }
          }
          ...
          // Explicitly set previous_purchase_id (don't use spread that might omit it)
          result.previous_purchase_id = previousPurchaseId;

          // Add optional fields
          if (globalIdsInfo) {
            result.global_ids_info = globalIdsInfo;
          }
          if (expiryLoss) {
            result.expiry_loss = expiryLoss;
          }
          if (renewalCountdown) {
            result.renewal_countdown = renewalCountdown;
          }

          // Debug for purchase 1774
          if (p.id.toString() === '1774') {
            console.log(`[DEBUG 1774] Raw:`, {
              raw_prev_pur: p.previous_purchase_id,
              converted: previousPurchaseId,
              in_result: result.previous_purchase_id,
  ...
  /**
   * @openapi
   * /api/v1/my-packages/{id}:
   *   get:
   *     tags:
   *       - My Packages
   *     summary: Get package purchase details by ID
   *     description: |
   *       Retrieve detailed information about a specific package purchase.
   *       Users can only view their own purchases.
   *     operationId: getMyPackageById
   *     security:
   *       - bearerAuth: []
   *     parameters:
   *       - in: path
  ...
  app.get('/:id', {
    preHandler: requireUser,
    schema: {
      description: 'Get package purchase details',
      tags: ['My Packages'],
    ...
    try {
      ...
      if (isExpired) {
        try {
          ...
          console.error('[my-packages/:id] Error calculating expiry loss:', error);
        }
        try {
          renewalCountdown = await PackageStatusService.calculateRenewalCountdown(purchase.id, userId);
        } catch (error) {
          console.error('[my-packages/:id] Error calculating renewal countdown:', error);
        }
      }
      ...
      const responseData: any = {
        ...
        renewed_at: purchase.renewed_at || null,
      };
