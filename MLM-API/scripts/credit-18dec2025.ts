 ...
 * for all eligible users for 18 Dec 2025.
 * 
 * This script reuses the exact same logic from creditDailyCommissions()
 * but with a fixed date (18 Dec 2025) instead of current date.
 * 
 * Usage: npx tsx scripts/credit-18dec2025.ts
 * 
...
// Fixed date: 18 Dec 2025
const creditDate = new Date('2025-12-18');
creditDate.setHours(0, 0, 0, 0);
const dateString = creditDate.toISOString().slice(0, 10); // "2025-12-18"
const daysInMonth = 31; // December 2025
const isLastDayOfMonth = false; // Dec 18 is not the last day

async function creditCommissionsFor18Dec2025() {
  console.log('🚀 Starting commission credit for 18 Dec 2025...');
    ...
    try {
      ...
      if (pkg && pkg.self_roi_percent && Number(pkg.self_roi_percent) > 0) {
        try {
          ...
          if (selfAmount > 0) {
            // Fixed idempotency key: daily:self:{purchaseId}:2025-12-18
            ...
            if (!existingSelf) {
              await addLedgerAndWallet({
                ...
                creditedAt: new Date('2025-12-18T00:00:00Z'), // Fixed date: 18 Dec 2025 00:00:00 UTC
              });
      ...
      if (pkg && pkg.global_ids && Number(pkg.global_ids) > 0) {
        try {
          ...
          // Calculate used IDs (up to 18 Dec 2025)
          ...
          // IMPORTANT: Count up to 18 Dec 2025, not current date
          const creditDateEnd = new Date('2025-12-18');
          ...
          const uniqueFirstPurchases = await prisma.purchases.findMany({
            where: {
              ...
              purchased_at: { 
                gt: startDate,
                lte: creditDateEnd // Up to 18 Dec 2025
              },
          ...
          if (globalAmount > 0) {
            // Fixed idempotency key: daily:global:{purchaseId}:2025-12-18
      ...
      for (const { ancestor_id, depth } of uplines) {
        ...
        try {
          ...
          if (dailyAmount > 0) {
            // Fixed idempotency key: daily:monthly:{purchaseId}:{uplineId}:{level}:2025-12-18
  ...
  console.log('');
  console.log('═══════════════════════════════════════════════════════════');
  console.log('📊 COMMISSION CREDIT SUMMARY FOR 18 DEC 2025');
  console.log('═══════════════════════════════════════════════════════════');
  ...
  console.log('');
  console.log('-- Check ledger entries for 18 Dec 2025');
  console.log(`SELECT 
  commission_type,
  COUNT(*) as count,
  SUM(amount) as total
