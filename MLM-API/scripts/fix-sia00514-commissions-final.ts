 ...
 * Update existing entries and fix duplicates
 */
...
async function main() {
  ...
  console.log('🔧 FIXING SIA00514 COMMISSIONS - Final Fix');
  console.log('='.repeat(80));
    ...
    for (const date of dates) {
      ...
      if (existingGlobals.length > 1) {
        ...
        for (let i = 1; i < existingGlobals.length; i++) {
          ...
          if (walletTxn) {
            await prisma.wallet_transactions.delete({ where: { id: walletTxn.id } });
          }
      ...
      if (existingGlobal) {
        ...
        if (Math.abs(difference) > 0.01) {
          console.log(`   📝 ${dateStr} GLOBAL: Updating ₹${existingAmount.toFixed(2)} → ₹${expectedGlobalAmount.toFixed(2)} (diff: ₹${difference.toFixed(2)}, Used IDs: ${expectedUsedIds})`);
