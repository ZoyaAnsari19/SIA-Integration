...
async function checkMissingMonthlyRoyalty() {
  console.log('🔍 Checking Missing Monthly Royalty (18 Dec 2025 - 23 Dec 2025)...\n');
  ...
  const referrerIds = Array.from(
    new Set(
      buyerUsers
        .filter((u) => u.referrer_user_id)
        .map((u) => u.referrer_user_id as unknown as bigint)
    )
  );
  ...
  const missingRoyaltyUsers: MissingRoyaltyUser[] = [];
  ...
  for (const purchase of purchases) {
    ...
    // Get actual monthly royalty received for this purchase
    ...
    if (isPurchase2x) {
      ...
      reason = 'Monthly royalty not credited (eligibility or code issue)';
    } else {
      reason = 'Royalty received';
    }

    // Only add if royalty is missing or less than expected
    if (actualMonthlyRoyalty === 0 && expectedMonthly > 0) {
      missingRoyaltyUsers.push({
  ...
  console.log(`   Purchases with referrers: ${missingRoyaltyUsers.length + purchases.length - missingRoyaltyUsers.length}`);
  console.log(`   Missing monthly royalty cases: ${missingRoyaltyUsers.length}\n`);

  // Group by reason
  const byReason = new Map<string, number>();
  missingRoyaltyUsers.forEach((u) => {
  ...
  console.log('\n' + '='.repeat(100));
  console.log('❌ Missing Monthly Royalty Details:\n');

  // Show first 50 cases
  for (const user of missingRoyaltyUsers.slice(0, 50)) {
  ...
  if (missingRoyaltyUsers.length > 50) {
    console.log(`\n   ... and ${missingRoyaltyUsers.length - 50} more cases\n`);
  }

  console.log('\n' + '='.repeat(100));
  console.log(`\n📝 Total Missing Monthly Royalty Cases: ${missingRoyaltyUsers.length}\n`);

  // Calculate total missing amount
  const totalMissingAmount = missingRoyaltyUsers.reduce(
    (sum, u) => sum + u.expected_monthly_royalty,
    0
  );
  console.log(`💰 Total Expected Monthly Royalty (per month): ₹${totalMissingAmount.toFixed(2)}\n`);
}

async function main() {
  try {
    await checkMissingMonthlyRoyalty();
  } catch (error) {
