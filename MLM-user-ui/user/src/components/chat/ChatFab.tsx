...
function formatActionResult(toolName: string | undefined, result: any, debug: boolean): string {
  ...
  if (tn === 'adminApproveWithdrawal' || tn === 'adminApproveWithdrawalsByDate') {
    const s = r?.summary;
    if (s?.approved != null) return `Withdrawals approved: ${s.approved}/${s.attempted || s.approved}.${details}`.trim();
...
async function streamChat(opts: {
  ...
  if (debug) {
    console.log('[chat] stream start', {
      url: `${opts.baseUrl}/chat/stream`,
      body: { message: opts.message, conversation_id: opts.conversationId, attachments: opts.attachments || [] },
    });
  }
  const res = await fetch(`${opts.baseUrl}/chat/stream`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${opts.token}`,
    },
    body: JSON.stringify({ message: opts.message, conversation_id: opts.conversationId, attachments: opts.attachments || [] }),
  });
  if (!res.ok || !res.body) {
    let detail = '';
    try {
    ...
    throw new Error(`Chat stream failed (${res.status})${detail ? `: ${detail}` : ''}`);
  }
...
async function streamConfirm(opts: {
  baseUrl: string;
  token: string;
  conversationId: string;
  ...
  if (debug) {
    console.log('[chat] confirm start', {
      url: `${opts.baseUrl}/chat/confirm`,
      body: { conversation_id: opts.conversationId, confirmation_token: opts.confirmationToken, confirm: opts.confirm },
    });
  }
  const res = await fetch(`${opts.baseUrl}/chat/confirm`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${opts.token}`,
    },
    body: JSON.stringify({ conversation_id: opts.conversationId, confirmation_token: opts.confirmationToken, confirm: opts.confirm }),
  });
  if (!res.ok || !res.body) {
    let detail = '';
    try {
    ...
    throw new Error(`Confirm stream failed (${res.status})${detail ? `: ${detail}` : ''}`);
  }
...
export function ChatFab(props: ChatFabProps) {
  ...
  async function onConfirm() {
    ...
    try {
      await streamConfirm({
        ...
        onEvent: (ev) => {
          if (ev.event === 'tool_result') {
            const toolName = (ev.data?.tool_name || pendingConfirm.toolName) as string | undefined;
            setMsgs((m) => [...m, { role: 'assistant', text: formatActionResult(toolName, ev.data?.result, debug) }]);
          }
        },
      });
      setPendingConfirm(null);
  ...
  async function onConfirmDecision(confirm: boolean) {
    const token = getAuthToken();
    if (!token || !pendingConfirm) return;
    setBusy(true);
    const debug = process.env.NEXT_PUBLIC_CHAT_DEBUG === 'true';
    ...
    <>
      {mode === 'floating' ? (
        <button
          ...
          className="fixed bottom-6 right-24 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-emerald-600 text-white shadow-[0_10px_25px_rgba(0,0,0,0.35)] hover:bg-emerald-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 focus:ring-offset-[var(--app-bg)] transition-colors"
      ...
      {open ? (
        <div
          className={[
            mode === 'floating'
              ? 'fixed bottom-6 right-6 z-50 h-[70vh] max-h-[640px] min-h-[520px] w-[92vw] max-w-[420px]'
              : 'w-full max-w-[960px] h-[75vh] min-h-[560px] mx-auto',
            'flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-[var(--card-bg)] shadow-[0_20px_60px_rgba(0,0,0,0.55)]',
            props.className || '',
          ].join(' ')}
        >
          <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <div className="text-sm font-semibold">SIA Assistant</div>
            </div>
            <button type="button" onClick={() => setOpen(false)} className="rounded-md p-2 hover:bg-white/5" aria-label="Close">
              <X className="h-5 w-5" />
            </button>
          </div>
            ...
            <div className="space-y-3">
                ...
                <div key={i} className={m.role === 'user' ? 'flex justify-end' : 'flex justify-start'}>
                  <div
                    className={[
                      ...
                      m.role === 'user' ? 'bg-emerald-600 text-white' : 'bg-white/5 text-white',
                      m.role === 'system' ? 'bg-amber-500/10 text-amber-200' : '',
                    ].join(' ')}
                  >
              ...
              {busy && statusText ? (
                <div className="flex justify-start">
                  <div className="max-w-[85%] rounded-2xl bg-white/5 px-3 py-2 text-sm text-white">
                    <div className="flex items-center gap-2">
                      <TypingDots />
              ...
              {pendingConfirm ? (
                <div className="rounded-xl border border-emerald-400/30 bg-emerald-500/10 p-3 text-sm">
                  <div className="font-semibold text-emerald-200">Confirmation required</div>
                  <div className="mt-1 text-white/80">
                    Tool: <span className="font-mono">{pendingConfirm.toolName}</span>
                  </div>
                  <pre className="mt-2 max-h-40 overflow-auto rounded-lg bg-black/30 p-2 text-xs text-white/80">
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    <button
                      ...
                      disabled={busy}
                      className="inline-flex items-center gap-2 rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-xs font-semibold text-white hover:bg-white/15 disabled:opacity-60"
                    >
                      No
                    </button>
                  </div>
          ...
          <div className="border-t border-white/10 p-3">
            {pendingAttachments.length ? (
              <div className="mb-2 flex items-center justify-between rounded-xl bg-white/5 px-3 py-2 text-xs text-white/80 ring-1 ring-white/10">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="h-10 flex-1 rounded-xl bg-white/5 px-3 text-sm outline-none ring-1 ring-white/10 focus:ring-2 focus:ring-emerald-500"
                disabled={busy}
              />
              ...
              <button
                ...
                disabled={busy}
                className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-white/5 ring-1 ring-white/10 hover:bg-white/10 disabled:opacity-60"
              ...
              <button
                type="button"
                onClick={() => {
                  if (busy || (!input.trim() && !pendingAttachments.length)) return;
                  const t = input.trim();
                  setInput('');
                  onSend(t);
                }}
            ...
            <div className="mt-2 text-[11px] text-white/40">
              Dev tip: call tools via <span className="font-mono">!tool getUserProfile {"{}"}</span>
            </div>
          </div>
        </div>
      ) : null}
